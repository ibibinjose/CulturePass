...
function offeringsFromRestaurant(r) {
    ...
    const href = `/r/${r.id}`;
    const out = [];
    for (const deal of (_a = r.deals) !== null && _a !== void 0 ? _a : []) {
        if (deal.isActive === false)
            continue;
        out.push(Object.assign(Object.assign({ id: `restaurant:${r.id}:deal:${deal.id}`, kind: 'restaurant_deal', domain: 'restaurant' }, base), { title: deal.title, subtitle: deal.discount, description: deal.description, imageUrl: r.imageUrl, validTo: deal.validTill, href }));
    }
    ((_b = r.menuHighlights) !== null && _b !== void 0 ? _b : []).forEach((line, idx) => {
        const title = line.trim();
        if (!title)
            return;
        out.push(Object.assign(Object.assign({ id: `restaurant:${r.id}:menu:${idx}`, kind: 'restaurant_menu_highlight', domain: 'restaurant' }, base), { title, subtitle: r.cuisine, imageUrl: r.imageUrl, href }));
    });
    return out;
}
function offeringsFromShop(s) {
    ...
    const href = `/s/${s.id}`;
    const out = [];
    for (const deal of (_a = s.deals) !== null && _a !== void 0 ? _a : []) {
        if (deal.isActive === false)
            continue;
        out.push(Object.assign(Object.assign({ id: `shopping:${s.id}:deal:${deal.id}`, kind: 'shopping_deal', domain: 'shopping' }, base), { title: deal.title, subtitle: deal.discount, description: deal.description, imageUrl: s.imageUrl, validTo: deal.validTill, href }));
    }
    return out;
}
...
function offeringsFromMovie(m, currency = 'AUD') {
    ...
    return ((_a = m.showtimes) !== null && _a !== void 0 ? _a : []).map((st) => (Object.assign(Object.assign({ id: `movie:${m.id}:showtime:${st.id}`, kind: 'movie_showtime', domain: 'movie' }, base), { title: m.title, subtitle: `${st.venueName} · ${st.date} ${st.time}`, description: m.description, imageUrl: poster, priceCents: showtimePriceCents(st.price), priceLabel: showtimePriceLabel(st.price, currency), currency, validFrom: st.date, href, externalUrl: st.bookingUrl })));
}
