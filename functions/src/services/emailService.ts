...
function buildDigestHtml(params: {
  ...
  const eventRows = params.events
    .map((e) => {
      const imgTag = e.imageUrl
        ...
        : '';
      const venueSpan = e.venue ? ' · ' + escapeHtml(e.venue) : '';
      return '<tr>' +
        '<td style="padding:12px 0;border-bottom:1px solid #f0f0f0;">' +
        imgTag +
        '<span style="display:inline-block;vertical-align:top;">' +
        '<strong style="font-size:15px;color:#1a1a1a;">' + escapeHtml(e.title) + '</strong><br/>' +
        '<span style="font-size:13px;color:#666;">' + escapeHtml(formatDate(e.date)) + venueSpan + '</span><br/>' +
        '<a href="' + escapeHtml(APP_URL) + '/e/' + escapeHtml(e.id) + '" style="font-size:13px;color:#9333EA;text-decoration:none;">View event →</a>' +
        '</span>' +
        '</td>' +
        '</tr>';
    })
  ...
  return '<!DOCTYPE html>' +
    ...
    '<a href="' + escapeHtml(APP_URL) + '/discover" style="display:inline-block;background:linear-gradient(135deg,#9333EA,#FF5E5B);color:#fff;text-decoration:none;padding:12px 28px;border-radius:100px;font-size:14px;font-weight:600;">Discover more →</a>' +
    '</div>' +
    '</td>' +
    '</tr>' +
    '<tr>' +
    ...
    '<a href="' + escapeHtml(APP_URL) + '/settings/notifications" style="color:#9333EA;text-decoration:none;">Manage preferences</a>' +
    '</p>' +
    '</td>' +
    '</tr>' +
    '</table>' +
