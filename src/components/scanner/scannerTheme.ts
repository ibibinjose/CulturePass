...
/** Per-mode accent — identity (indigo) vs gate tickets (terracotta) */
export const SCAN_MODE_ACCENT: Record<ScanMode, string> = {
  culturepass: Luxe.colors.indigo,
  tickets: Luxe.colors.terracotta,
};
...
export function scanAccentGradient(mode: ScanMode): [string, string] {
  return mode === 'tickets' ? Luxe.gradients.terracottaBronze : Luxe.gradients.emeraldIndigo;
}

export function scanSubmitColor(mode: ScanMode): string {
  return mode === 'tickets' ? Luxe.colors.terracotta : Luxe.colors.indigo;
}