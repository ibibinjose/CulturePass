 ...
 *
 * Handles all short link prefixes: /e/, /c/, /b/, /v/, /cpu/ (preferred for users + usernames), /u/, /o/, /t/
 *
 * Usage:
...
const ROUTE_MAP: Record<DeepLinkEntityType, (id: string) => string> = {
  ...
  user: (id) => {
    // Prefer branded CPU path for CulturePass User IDs and handles (/cpu/username)
