  ...
  return (
    ...
    >
        ...
        <View style={[styles.imageContainer, { backgroundColor: colors.backgroundSecondary }]}>
          ...
          {isLive ? (
            <View style={styles.liveBadge}>
              <LinearGradient
                colors={[CultureTokens.coral, '#D13431']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={StyleSheet.absoluteFill}
              />
              <Ionicons name="pulse" size={12} color={colors.eventDateOnMedia} style={{ marginRight: 4 }} />
              <Text style={[styles.liveBadgeText, { color: colors.eventDateOnMedia }]}>LIVE</Text>
            </View>
          ) : null}
          ...
          {event.publisherProfileId ? (
            <View style={styles.publisherLogoWrap}>
              <EventPublisherLogo profileId={event.publisherProfileId} size={46} />
            </View>
          ) : null}
        </View>
      ...
      <View style={styles.floatingActions}>
        <Pressable
          style={({ pressed }) => [styles.actionButtonFloating, pressed && { opacity: 0.7 }]}
        ...
        >
          <View
            style={[
              styles.actionIconBg,
              { backgroundColor: colors.backgroundSecondary, borderColor: colors.eventDateOnMedia },
              liked && { backgroundColor: CultureTokens.coral + '20' },
            ]}
          >
            <Ionicons
              name={liked ? 'heart' : 'heart-outline'}
              size={18}
              color={liked ? CultureTokens.coral : colors.textSecondary}
            />
          </View>
        </Pressable>
      ...
      {canEdit ? (
        <View style={styles.crudActionsRow}>
          <Pressable
            onPress={handleEdit}
            style={({ pressed }) => [styles.crudBtn, { backgroundColor: colors.surface, borderColor: colors.borderLight }, pressed && { opacity: 0.7 }]}
            accessibilityRole="button"
      ...
      {showUndo ? (
        <View style={[styles.snackbar, { backgroundColor: colors.surfaceElevated }]} accessibilityLiveRegion="polite">
          <Text style={[styles.snackbarText, { color: colors.text }]}>Event deleted</Text>
