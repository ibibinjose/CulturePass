...
import { HostspaceQuickLinks } from '@/components/hostspace/HostspaceQuickLinks';
import {
  buildHostspaceManageHref,
  resolveHostspaceManageTab,
} from '@/lib/hostspaceDeeplinks';
...
export function HostspaceManagePanel() {
  const colors = useColors();
  ...
  const searchParams = useLocalSearchParams<{ tab?: string; manageTab?: string }>();
  const [showDraftModal, setShowDraftModal] = useState(false);
  const manageTab = useMemo(
    () => resolveHostspaceManageTab(searchParams),
    [searchParams.tab, searchParams.manageTab],
  );

  const handleManageTabChange = useCallback((tab: HostspaceManageTab) => {
    router.replace(buildHostspaceManageHref(tab) as never);
  }, []);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        <HostspaceHubNav activePanel="manage" hPad={hPad} />
      </HostspaceStickyBar>
